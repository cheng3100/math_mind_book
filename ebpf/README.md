...
## 13. VKMS/QEMU TASK 1/2 使用指南

本节说明 `ebpf/vkms_lab/` 下 TASK 1/2 脚本的具体使用方式。完整设计和问题记录分别见：

- `ebpf/bpftracec_start_lld.md`
- `ebpf/task_note.md`
...
```text
ebpf/vkms_lab/
├── task1_prepare_busybox.sh      # TASK 1: 下载并编译静态 BusyBox
├── task1_build_linux.sh          # TASK 1: 编译 Linux bzImage 和 vkms.ko
├── task1_run_qemu.sh             # TASK 1/2: 启动 QEMU guest，加载 VKMS，执行 smoke test
├── task2_fetch_drm_tools.sh      # TASK 2: 下载 libdrm / IGT / kmod / pciutils 源码
└── task2_build_drm_tools.sh      # TASK 2: 编译 modetest 和 IGT KMS tests
```
...
默认生成目录都在 `ebpf/vkms_lab/` 下，并已被 `.gitignore` 忽略：

```text
external/          # TASK 2 下载的源码：libdrm / IGT / kmod / pciutils
task1-runtime/     # TASK 1 的 kernel build、BusyBox、initramfs
tools-build/       # TASK 2 的 Meson/Make 构建目录
tools-install/     # TASK 2 安装给 QEMU guest 使用的工具
rootfs/            # 预留 rootfs 实验目录
```
...
它只负责 host 级工具，例如 compiler、QEMU、Meson/Ninja、kernel build 依赖和常见开发库。BusyBox、libdrm、IGT、kmod、pciutils 这类实验材料由 TASK 1/2 脚本按源码下载和构建。

当前脚本已经尽量把 `libkmod` 和 `libpci` 也通过源码本地构建解决；如果 host 工具本身缺失，再运行 `env.sh`。

### 13.3 TASK 1：准备 BusyBox

`task1_prepare_busybox.sh` 从 BusyBox 官方源码包下载并编译 static busybox，供 QEMU initramfs 使用：
...
```text
ebpf/vkms_lab/task1-runtime/busybox/install/busybox
```

通常不需要手动运行它，因为 `task1_run_qemu.sh` 如果发现默认 busybox 不存在，会自动调用该脚本。

### 13.4 TASK 1：编译 Linux 和 VKMS
...
./task1_build_linux.sh \
  /home/cheng/work/os/linux/linux \
  ./task1-runtime/linux-build
```

参数含义：

```text
argv[1] Linux 源码目录，默认 /home/cheng/work/os/linux/linux
argv[2] kernel O= 输出目录，默认 ./task1-runtime/linux-build
```
...
- BPF / BTF / kprobe / ftrace / tracefs / debugfs / bpffs
- DRM / VKMS / virtio / 9p
...
vkms_module=.../drivers/gpu/drm/vkms/vkms.ko
```

### 13.5 TASK 1：启动 QEMU 并加载 VKMS
...
```bash
cd ebpf/vkms_lab
TASK1_SMOKE_TEST=1 ./task1_run_qemu.sh
```
...
```text
argv[1] Linux 源码目录，默认 /home/cheng/work/os/linux/linux
argv[2] kernel build 目录，默认 ./task1-runtime/linux-build
argv[3] runtime 工作目录，默认 ./task1-runtime
```

guest 内脚本会：

- 挂载 `proc`、`sysfs`、`devtmpfs`、`tracefs`、`debugfs`、`bpffs`；
- 通过 9p 挂载仓库目录、kernel build 目录和 `tools-install/`；
- 执行 `insmod /kbuild/drivers/gpu/drm/vkms/vkms.ko ...`；
- 输出 `/dev/dri`；
- 从 `available_filter_functions` 中列出 VKMS/DRM atomic 可 probe 函数。

`TASK1_SMOKE_TEST=1` 会检查 `/dev/dri/card0`，成功后自动关机退出 QEMU。

### 13.6 TASK 2：下载 modetest/IGT 相关源码
...
```bash
LIBDRM_COMMIT=<commit> \
IGT_COMMIT=<commit> \
KMOD_COMMIT=<commit> \
PCIUTILS_COMMIT=<commit> \
  ./task2_fetch_drm_tools.sh
```

当前自检使用过的 commit 记录在 `ebpf/task_note.md`。

### 13.7 TASK 2：编译 modetest 和 IGT
...
- 编译 libdrm；
- 把 libdrm 的 `modetest` 安装到 `tools-install/bin/modetest`；
- 如果系统缺 `libkmod`，从 `external/kmod` 本地构建 `libkmod`；
- 如果系统缺 `libpci`，从 `external/pciutils` 本地构建 `libpci`；
- 编译并安装 IGT tests 到 `tools-install/libexec/igt-gpu-tools/`。

关键产物：

```text
tools-install/bin/modetest
tools-install/libexec/igt-gpu-tools/kms_atomic
tools-install/libexec/igt-gpu-tools/kms_flip
tools-install/libexec/igt-gpu-tools/kms_plane
tools-install/libexec/igt-gpu-tools/kms_writeback
tools-install/libexec/igt-gpu-tools/kms_getfb
tools-install/libexec/igt-gpu-tools/kms_addfb_basic
```

host 上可以先做轻量检查：

```bash
LD_LIBRARY_PATH=./tools-install/lib:./tools-install/lib64:./tools-install/lib/x86_64-linux-gnu:./tools-install/libexec/igt-gpu-tools \
  ./tools-install/libexec/igt-gpu-tools/kms_getfb --list-subtests
```

### 13.8 TASK 2：在 QEMU guest 内运行触发器 smoke test

TASK 2 smoke test 复用 `task1_run_qemu.sh`，因为它负责启动 guest 和加载 VKMS：

```bash
cd ebpf/vkms_lab
TASK1_SMOKE_TEST=1 TASK2_SMOKE_TEST=1 ./task1_run_qemu.sh
```

guest 内会额外执行：

```bash
/tools/bin/modetest -M vkms -c
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_getfb \
...
- `modetest` 能枚举 VKMS connector；
- IGT `kms_getfb/getfb-handle-valid` 输出 `SUCCESS`，能触发 DRM framebuffer ioctl 路径；
- `task1_run_qemu.sh` 在 `TASK2_SMOKE_TEST=1` 时会自动把动态 loader 和必要系统共享库加入 initramfs；`libdrm/libigt/libkmod/libpci` 仍来自 `/tools` 挂载的源码构建产物；
- guest 优先使用 `/tools`；如果 `/tools` 不可用，脚本会尝试回退到整体仓库挂载下的 `/mnt/tools-install`；
- `TASK1_SMOKE_TEST=1` 最后自动 poweroff。

更复杂的测试可以在交互 guest shell 中手动运行：

```sh
/tools/bin/modetest -M vkms -c
/tools/bin/modetest -M vkms -p
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_addfb_basic --device drm:/dev/dri/card0 --run-subtest basic
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_atomic --device drm:/dev/dri/card0 --list-subtests
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_flip --device drm:/dev/dri/card0 --list-subtests
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_plane --device drm:/dev/dri/card0 --list-subtests
IGT_FORCE_DRIVER=vkms /tools/libexec/igt-gpu-tools/kms_writeback --device drm:/dev/dri/card0 --list-subtests
```

这些命令用于后续配合 bpftrace 抓取：

```text
modetest / IGT
  -> DRM ioctl
  -> drm_mode_* / drm_atomic_*
  -> vkms_* callback
  -> vblank / composer / writeback
```

## 14. 与 GPU KMD 工作的最终映射
